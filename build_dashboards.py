...
# ---------------------------------------------------------------- shared brand CSS
BRAND_CSS = """
@import url('https://fonts.googleapis.com/css2?family=Hanken+Grotesk:wght@400;500;600;700;800&family=Spectral:ital,wght@0,300;0,400;0,500;0,600;1,400;1,500&family=Spline+Sans+Mono:ital,wght@0,400;0,500;0,600;1,400&display=swap');
:root{
  ...
  --display:'Hanken Grotesk',system-ui,sans-serif; --body:'Spectral',Georgia,serif;
...
(function drawTrend(){
  ...
  // mark peaks (Nov)
  let marks='';
  mo.forEach((d,i)=>{ if(d.month.endsWith('-11')){ marks+=`<circle cx="${x(i).toFixed(0)}" cy="${y(d.revenue).toFixed(0)}" r="3.5" fill="#2f4ba8"/><text x="${x(i).toFixed(0)}" y="${(y(d.revenue)-9).toFixed(0)}" text-anchor="middle" font-family="'Spline Sans Mono',monospace" font-size="11" fill="#2f4ba8">${d.month.slice(2,4)==='09'?'Nov 09':'Nov '+d.month.slice(2,4)}</text>`; }});
